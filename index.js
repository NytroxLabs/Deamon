 ...
 *               \___/
 * 
 *  Nytrox Daemon v0.1-beta1
 *  (c) 2024 Zenpai & Shifu
 * 
*/

/**
 * @fileoverview Main entry file for the Nytrox Daemon. This module sets up an
...
app.get('/', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Docker is not running - Nytrox DAEMON will not function properly.' });
    }
});
...
setTimeout(function (){
  server.listen(port, () => {
    log.info('NytroxDaemon is listening on port ' + port);
  });
}, 2000);